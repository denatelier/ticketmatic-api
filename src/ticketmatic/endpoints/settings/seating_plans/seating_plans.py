...
def get_list(client: Client, params: SeatingPlanQuery | dict | None = None) -> SeatingPlansList:
    return crud_get_list(client, _URL, SeatingPlanQuery, params, SeatingPlansList, _FIELDS)
...
def save_lock_templates(client: Client, id: int, data: list[LockTemplate | dict]) -> list[LockTemplate]:
    body = [item.to_dict() if isinstance(item, LockTemplate) else LockTemplate.from_dict(item).to_dict() for item in data]
    req = client.new_request("POST", f"{_ITEM}/locktemplates")
    req.add_parameter("id", id)
    req.set_body(body)
    return unpack_array(LockTemplate, req.run())

def get_seat_description_templates(client: Client, id: int) -> list[SeatDescriptionTemplate]:
    req = client.new_request("GET", f"{_ITEM}/seatdescriptiontemplates")
    req.add_parameter("id", id)
    return unpack_array(SeatDescriptionTemplate, req.run())

def save_seat_description_templates(client: Client, id: int, data: list[SeatDescriptionTemplate | dict]) -> list[SeatDescriptionTemplate]:
    body = [item.to_dict() if isinstance(item, SeatDescriptionTemplate) else SeatDescriptionTemplate.from_dict(item).to_dict() for item in data]
