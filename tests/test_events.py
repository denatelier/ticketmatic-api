...
def test_batch(tm_client):
    ...
    event2 = events.create(tm_client, {
        "contingents": [{"amount": 100}],
        "locationid": 1,
        "name": "Example2",
    })
...
def test_get_conditions(tm_client):
    ...
    assert req.prices.contingents[0].pricetypes[0].saleschannels[0].conditions[0].type == "orderticketlimit"
...
def test_get_draft(tm_client):
    ...
    result = events.get_list(tm_client, EventQuery(
...
def test_lock_unlock_tickets(tm_client):
    ...
    events.lock_tickets(tm_client, result.data[0].id, {
        "locktypeid": 1,
        "ticketids": [tickets[0]["id"], tickets[1]["id"]],
    })

    events.unlock_tickets(tm_client, result.data[0].id, {
        "ticketids": [tickets[0]["id"], tickets[1]["id"]],
    })
...
def test_update_seat_rank_for_tickets(tm_client):
    ...
    events.update_seat_rank_for_tickets(tm_client, result.data[0].id, {
        "seatrankid": 3,
        "ticketids": [tickets[0]["id"], tickets[1]["id"]],
    })
